...
from .models import item,order,orderitem,Customer,HouseHold_item_category,Kitchen_item_category,Shipping,Profile,Registered_shop,Shop_categories
    ...
    if request.user.is_authenticated:
        ...
        try:
           ...
           context = {
               ...
               'LocalServicesShop':Registered_shop.objects.filter(Shop_category=LId.id),
...
@login_required
def Myshop_profile_view(request,pk):
...
class Registered_shopUpdateView(UpdateView):
    ...
    fields = {'shop_img', 'shop_name','Retailer_name', 'phone_no', 'shop_address', 'city', 'pin_no', 'state','Shop_category'}
...
def search(request):
    ...
    return render(request,'grocery/search.html',{'items':items})
